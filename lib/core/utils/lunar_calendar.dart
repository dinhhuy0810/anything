...
class LunarDate {
  ...
  /// Can Chi của năm âm lịch, ví dụ: "Giáp Thìn".
  String get yearCanChi {
    final can = _canList[(year + 6) % 10];
    final chi = _chiList[(year + 8) % 12];
    return '$can $chi';
  }

  @override
  String toString() => '$day/$month${isLeap ? ' (nhuận)' : ''}';
}

/// Thuật toán chuyển đổi Dương lịch sang Âm lịch cho Việt Nam,
/// dựa trên công thức thiên văn (múi giờ UTC+7) được dùng phổ biến
/// trong các ứng dụng lịch Việt Nam.
class LunarCalendar {
  LunarCalendar._();
  ...
  static LunarDate solarToLunar(int dd, int mm, int yy) {
    final result = _convertSolar2Lunar(dd, mm, yy, _vnTimeZone);
    return LunarDate(
      day: result[0],
      month: result[1],
      year: result[2],
  ...
  static int _sunLongitude(int dayNumber, double timeZone) {
  ...
  static int _getLunarMonth11(int yy, double timeZone) {
    ...
    final sunLong = _sunLongitude(nm, timeZone);
    if (sunLong >= 9) {
  ...
  static int _getLeapMonthOffset(int a11, double timeZone) {
    ...
    var last = 0;
    var i = 1;
    var arc = _sunLongitude(_getNewMoonDay(k + i, timeZone), timeZone);
    do {
      last = arc;
      i++;
      arc = _sunLongitude(_getNewMoonDay(k + i, timeZone), timeZone);
    } while (arc != last && i < 14);
    return i - 1;
  }
